...
def main():
    ...
    conn = sqlite3.connect('laptop-price-spesification.db')
    cursor = conn.cursor()

    # Execute SQL script to create tables for the AI RSS Feed system
    execute_sql_script(cursor, 'table_laptop.sql')
    conn.commit()

    # Execute SQL script to insert mock data for the AI RSS Feed system
    execute_sql_script(cursor, 'price.sql')
    conn.commit()
